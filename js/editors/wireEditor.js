class WireEditor{
    ...
    display(){
        ...
        if(this.hovered){
            this.hovered.draw(this.ctx, {color:"blue", fill:true});
        }
        if(this.selected){
            const intent = this.hovered ? this.hovered:this.mouse;
            new Segment(this.selected, intent).draw(ctx, {color : "white", dash:[3, 3]});
            this.selected.draw(this.ctx, {color:"red",outline:true});
        }
    }
    #addEventListeners(){
    ...
    #handleMouseDown(event){
        ...
        if(event.button == 2){  //right click
            ...
            else if(this.hovered && this.hovered.owner != "component"){
    ...
    #handleMouseMove(event){
        ...
        if(this.dragging == true && this.selected.owner != "component"){
